...
export default function CoachMatchEdit({ match, team_names, tournaments }: Props) {
  const { data, setData, patch, processing, errors } = useForm({
    opponent:       match.opponent ?? "",
    date:           match.date ?? "",
    kickoff_time:   match.kickoff_time ?? "",
    home:           match.home ? "true" : "false",
    team_name:      match.team_name ?? team_names[0] ?? "Primera",
    tournament_id:  match.tournament_id?.toString() ?? tournaments[0]?.id?.toString() ?? "",
    points_for:     match.points_for?.toString() ?? "",
    points_against: match.points_against?.toString() ?? "",
  })

  function submit(e: React.FormEvent) {
    e.preventDefault()
    patch(`/coach/matches/${match.id}`)
  }

  return (
    <CoachLayout>
      <Head title={`Editar · vs. ${match.opponent}`} />

      <div className="bg-white border-b border-gray-100 px-4 pb-4 pt-8">
        <Link href={`/coach/matches/${match.id}`} className="flex items-center gap-1 text-bordo-600 hover:text-bordo-800 mb-4 text-sm">
          <ArrowLeft className="h-4 w-4" /> Volver al partido
        </Link>
        <h1 className="text-2xl font-bold text-gray-900">Editar Partido</h1>
        <p className="text-sm text-gray-400 mt-0.5">vs. {match.opponent}</p>
      </div>
      ...
      <div className="px-4 pt-4 pb-6 bg-gray-50 min-h-full">
        <form onSubmit={submit} className="space-y-4">
          ...
          {/* Resultado */}
          <div className="rounded-2xl bg-white p-4 border border-gray-100 shadow-sm space-y-4">
            <h2 className="font-semibold text-gray-800 border-b border-gray-100 pb-2">Resultado</h2>
            <div className="grid grid-cols-2 gap-3">
              <div>
