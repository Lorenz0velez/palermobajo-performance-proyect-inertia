...
export default function CoachTrainingsEdit({ training, selected_role_ids, functional_roles }: Props) {
    ...
    <CoachLayout>
      ...
      <div className="bg-white border-b border-gray-100 px-4 pb-4 pt-10">
        <div className="flex items-center gap-3">
          <Link href={`/coach/trainings/${training.id}`} className="text-gray-400 hover:text-gray-700">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <h1 className="text-xl font-bold text-gray-900">Editar Entrenamiento</h1>
        </div>
      </div>

      <form onSubmit={(e) => { e.preventDefault(); put(`/coach/trainings/${training.id}`) }} className="px-4 pt-5 pb-8 space-y-4">
        ...
        <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-5 space-y-3">
          ...
          {!data.pdf_file && training.pdf_url && (
            <a
              href={training.pdf_url}
              target="_blank"
              rel="noreferrer"
              className="flex items-center gap-1.5 text-xs text-bordo-600 hover:text-bordo-800"
            >
